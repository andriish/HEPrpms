...
TDatime gen1_timestamp;
TDatime gen2_timestamp;
...
/*		if (strcmp(key_obj->GetClassName(),"TDecayMode")==0 &&
	  ...
	  if (generator==1)
	  {
	    ...
	    gen1_timestamp = f->GetCreationDate();
	  }
	  else
	  {
	    gen2           = (GenerationDescription*)key_dir->ReadObj();
	    gen2_timestamp = f->GetCreationDate();
	  }
	}
...
//	    h2->Sumw2();
	    ...
	    // Scale histograms. This option does not affect SDP calculation
	    if(Setup::scale_histograms)
	    {
	      double w1 = dr->h1->GetSumOfWeights();
	      double w2 = dr->h2->GetSumOfWeights();
	      if     (w1>w2) dr->h2->Scale( w1/w2 );
	      else if(w2>w1) dr->h1->Scale( w2/w1 );
	    }
