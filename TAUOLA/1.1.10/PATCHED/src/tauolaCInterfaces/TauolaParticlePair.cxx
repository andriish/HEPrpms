...
#include <math.h>
#include <iostream>
//static   double transcosTheta;


namespace Tauolapp
{
  extern "C" { double transcosTheta ; double transinvariant_mass;
    int transincoming_pdg_id;
    double transR11, transR22, transWT11, transWT22;
    double trans_h_tau_minus[4]; double trans_h_tau_plus[4];}
  //static  double transcosTheta;
...
void TauolaParticlePair::decayTauPair(){
  ...
  for(double weight=0; weight < Tauola::randomDouble();){
    ...
    double sinv=sqrt((tau_minus->getE()+tau_plus->getE())*(tau_minus->getE()+tau_plus->getE())
                    -(tau_minus->getPx()+tau_plus->getPx())*(tau_minus->getPx()+tau_plus->getPx())
                    -(tau_minus->getPy()+tau_plus->getPy())*(tau_minus->getPy()+tau_plus->getPy())
		    -(tau_minus->getPz()+tau_plus->getPz())*(tau_minus->getPz()+tau_plus->getPz()) );
    //   cout<<" --------------------tauP tauM --------------------------------sqrt(s)="<<sinv<<" "<<  tau_plus->getPdgID()<<"  "<<  tau_minus->getPdgID()<<endl;
    //	cout<<"taum -(E,x,y,z)=" << tau_minus->getE() <<" "  << tau_minus->getPx() <<" "  << tau_minus->getPy() <<" "  << tau_minus->getPz()  <<endl;
    //	cout<<"taup -(E,x,y,z)=" << tau_plus->getE() <<" "  << tau_plus->getPx() <<" "  << tau_plus->getPy() <<" "  << tau_plus->getPz()  <<endl;
    if(h_tau_plus[0]<1.3 && h_tau_minus[0]<1.3) double asymi=(tau_minus->getE()- tau_plus->getE())/(	tau_minus->getE()+ tau_plus->getE());
    }
    // 	cout<<" longitudinal energy asymmetry for the event =" << asymi <<endl;
    //	cout<<" elementy  Rij (0,0) (3,3) (0,3) (3,0)  =" <<endl;
    //  cout<<"                                         "<< m_R[0][0]<<" " <<m_R[3][3]<<" " << m_R[0][3]<<" " <<m_R[3][0] <<endl;
    //   cout<<" elementy  Rij (1,1) (2,2) (1,2) (2,1)  =";// <<endl;	
    //	cout<<"                                         "<< m_R[1][1]<<" " <<m_R[2][2]<<" " << m_R[1][2]<<" " <<m_R[2][1] <<endl;
	//	cout<<"h^-(0,1,2,3)=" << h_tau_minus[0] <<" "  << h_tau_minus[1] <<" "  << h_tau_minus[2] <<" "  << h_tau_minus[3]  <<endl;
	//	cout<<"h^+(0,1,2,3)=" << h_tau_plus[0]  <<" "  << h_tau_plus[1]  <<" "  << h_tau_plus[2]  <<" "  << h_tau_plus[3]   <<endl;
	//	cout<<" ----------------------------------------------------------- " <<endl; 
    //calculate the weight
	transR11=m_R[1][1];
	transR22=m_R[2][2];
	transWT11=m_R[1][1]* h_tau_minus[1]* h_tau_plus[1];
	transWT22=m_R[2][2]* h_tau_minus[2]* h_tau_plus[2];
	trans_h_tau_minus[3]= h_tau_minus[3];
	trans_h_tau_minus[1]= h_tau_minus[1];
	trans_h_tau_minus[2]= h_tau_minus[2];
	trans_h_tau_plus[3]= h_tau_plus[3];
	trans_h_tau_plus[1]= h_tau_plus[1];
	trans_h_tau_plus[2]= h_tau_plus[2];
    weight=0;
    for(int i =0; i<4; i++){
      for(int j=0; j<4; j++)
    ...
    weight = weight/4.0;
    //  cout<<"i z tego jest wt/4, weight= " << weight   <<endl;
  }
  double wthel[4]={0};
...
// cout<<" koniec pracy dla  decayTauPair"   <<endl;
}
...
void TauolaParticlePair::recalculateRij(int incoming_pdg_id, int outgoing_pdg_id, double invariant_mass_squared, double cosTheta){
  ...
  for (int i=0; i<4; i++){
    for (int j=0; j<4; j++){
      //     std::cout << "---petla i,j---"<<i<<" "<<j<<std::endl;
      ...
      //if(i==1&&j==1) { std::cout << "echh i,j=1 b11,b22,b21,b12= "<<b11<<" "<<b22<<" "<<b21<<" "<<b12<< std::endl;
      //}
    } // for(j)
  } // for(i)
  //std::cout << "---tutaaj---"<<std::endl;

  transcosTheta= cosTheta;
  transincoming_pdg_id=incoming_pdg_id;
  transinvariant_mass=sqrt(invariant_mass_squared);
  //std::cout << "transcosTheta= "<<transcosTheta<<std::endl;
  //std::cout << "uzywamy cosTheta, invariant_mass, incoming_pdg_id= "<< cosTheta<<" "<<sqrt(invariant_mass_squared)<<" "<< incoming_pdg_id<<std::endl;
  //std::cout << "testy m_R(1,1)(2,2)=: " << m_R[1][1]<<" "<<m_R[2][2]<<" refik= "<< (1-cosTheta*cosTheta)/(1+cosTheta*cosTheta+2*cosTheta)<<std::endl;
  //  h1->Fill(cosTheta);
  ...
  w0 = b11*(1-remnant)*(1-remnantc) + b21*remnant*(1-remnantc)
     + b12*(1-remnant)*remnantc     + b22*remnant*remnantc;
  // std::cout << "---tutaaj setEW---"<<std::endl;
  Tauola::setEWwt(w,w0);
  //std::cout << "---tutaaj po setEW---"<<std::endl;  
}
