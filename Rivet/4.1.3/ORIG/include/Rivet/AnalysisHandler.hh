...
namespace Rivet {
  ...
  public:
    ...
    void setAnnotation(const std::string& name, T&& value) {
      _beaminfo->setAnnotation<T>(name, std::forward<T>(value));
    }
  ...
  private:
    ...
    bool _checkBeams{true};

    /// @brief Flag whether every input event should be treated as independent
    ///
    /// @note Setting to true makes Rivet _not_ treat repeated event numbers as indicating correlated event groups.
    bool _ignoreEventGroups{false};

    /// Flag to check if multiweights should be ignored
    bool _skipMultiWeights{false};
