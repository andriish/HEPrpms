...
namespace Rivet {
  ...
  void AnalysisHandler::init(const GenEvent& ge) {
    ...
    // Set niche behavioural flags from env variables
    // _stripHepMC = ( getEnvParam("RIVET_STRIP_HEPMC", string("NOOOO") ) != "NOOOO" );
    _ignoreEventGroups = ( getEnvParam("RIVET_IGNORE_EVTGROUPS", string("0") ) != "0" );
  ...
  void AnalysisHandler::analyze(GenEvent& ge) {
    ...
    if (_ignoreEventGroups || _eventNumber != ge.event_number()) {
      collapseEventGroup();
      _eventNumber = ge.event_number();
  ...
  void AnalysisHandler::loadAOs(const map<string, YODA::AnalysisObjectPtr>& allAOs,
    ...
    const string beaminfokey("/TMP/_BEAMPZ");
    auto aobeam = allAOs.find(beaminfokey);
    if (aobeam == allAOs.end()) {
      ...
      copyAO(aobeam->second, _beaminfo, 1.0);
    }
    ...
    for (const auto& pair : allAOs) {
      if (pair.first == beaminfokey)  continue;
