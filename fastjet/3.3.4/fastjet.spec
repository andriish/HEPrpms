...
%else
%configure --enable-allplugins --enable-pyext 
%endif
%make_build
%endif
%if 0%{?suse_version}
...
%if %{?rhel}%{!?rhel:0} == 7
 ...
 cd pyinterface  
 make clean 
 make  PYTHON=%{__python3_other}  pythondir=%{python3_sitearch} 
 %make_install PYTHON=%{__python3_other} pythondir=%{python3_other_sitearch} 
  rm -f %{buildroot}%{python3_other_sitearch}/*.a
  rm -f %{buildroot}%{python3_other_sitearch}/*.la
 )
%endif
...
%{python3_other_sitearch}/*.p*
%{python3_other_sitearch}/_%{name}*.so*
