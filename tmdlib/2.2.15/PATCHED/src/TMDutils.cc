...
inline std::vector<std::string> split(const std::string &s, const std::string &sep)
{
    ...
    while (true)
    {
        ...
        if (!s.empty())
            rtn.push_back(s);              // Don't insert "empties"
        tmp.replace(0, delim_pos + 1, ""); // Remove already-processed part
    }
    if (!tmp.empty())
        rtn.push_back(tmp); // Don't forget the trailing component!
    return rtn;
}
...
double TMD::stringToDouble(const std::string &strValue, double defaultValue)
{
    if (strValue.empty())
        return defaultValue;
    try
    {
        size_t pos;
        double result = std::stod(strValue, &pos);
    ...
    {
        std::cout << "[TMDLIB2] the input string is not of type double! " << e.what() << std::endl;
    }
    return defaultValue;
}
int TMD::stringToInt(const std::string &strValue, int defaultValue)
{
    if (strValue.empty())
        return defaultValue;
    try
    {
        size_t pos;
        int result = std::stoi(strValue, &pos);
        if (pos == strValue.length())
        {
            return result;
        }
    ...
    {
        std::cout << "[TMDLIB2] the input string is not of type integer! " << e.what() << std::endl;
    }
    return defaultValue;
}

bool TMD::DoesFileExist(const std::string &name)
{
...
void TMD::TMDinfo(const std::string &name)
{
    ...
    */
    ...
    if (found != std::string::npos)
    {
        pdfdir = "ccfm-JH-2013-set1/";
    }
    found = name.find("ccfm-JH-2013-set2");
    if (found != std::string::npos)
    {
        pdfdir = "ccfm-JH-2013-set2/";
    }
    ...
    if (found != std::string::npos)
    {
        pdfdir = "PB-NLO-HERAI+II-2018-set1/";
    }
    found = name.find("PB-NLO-HERAI+II-2018-set1-q0");
    if (found != std::string::npos)
    {
        pdfdir = "PB-NLO-HERAI+II-2018-set1-q0/";
    }
    found = name.find("PB-NLO-HERAI+II-2018-set2");
    if (found != std::string::npos)
    {
        pdfdir = "PB-NLO-HERAI+II-2018-set2/";
    }
    found = name.find("PB-NLO-HERAI+II-2018-set2-q0");
    if (found != std::string::npos)
    {
        pdfdir = "PB-NLO-HERAI+II-2018-set2-q0/";
    }
    ...
    if (filepath.empty())
        std::cout << "Empty PDF file name given to Info::load  " << filepath << std::endl;
...
// comment: past behavior leads to crash if setDesc was not found, and no clear message was shown.
std::string TMD::TMDgetDesc()
{
    std::string description = getValue(TMDdict, "SetDesc", "NOT_FOUND");
    if (description == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] Having SetDesc is necessary for the info file");
    }
    return description;
}
// comment: past behavior leads to crash if SetIndex was not found, and no clear message was shown.
std::string TMD::TMDgetIndex()
{
    /** TMDlib helper to return SetIndexfrom info file */
    std::string SetIndex = getValue(TMDdict, "SetIndex", "NOT_FOUND");
    if (SetIndex == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] SetIndex is necessary for the info file");
    }
    return SetIndex;
}
std::string TMD::TMDgetScheme()
{
    ...
    std::string TMDScheme = getValue(TMDdict, "TMDScheme", "");
    if (NewFormat == 1 && TMDScheme == "")
    {
        throw std::runtime_error("[TMDLIB2] TMDScheme is necessary for the info file");
    }
    return TMDScheme;
}

std::string TMD::TMDgetReference()
{
    /** TMDlib helper to return Scheme for TMD evolution from info file */
    std::string Reference = getValue(TMDdict, "Reference", "NOT_FOUND");
    if (Reference == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] Reference is necessary for the info file");
    }
    return Reference;
}
std::string TMD::TMDgetAuthors()
{
    /** TMDlib helper to return Scheme for TMD evolution from info file */
    std::string Authors = getValue(TMDdict, "Authors", "NOT_FOUND");
    if (Authors == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] Authors is necessary for the info file");
    }
    return Authors;
}

// comment: there is no standard. Checking by eye, I noticed some info file does not have AlphaS_QCDlam
double TMD::TMDgetLam4()
{
    /** TMDlib helper to return QCDlam from info file */
    return stringToDouble(getValue(TMDdict, "AlphaS_QCDlam"));
}
// comment: currently if NumFlavors does not exist, this function is crashing. so I think returning DEFAULT_INVALID_KEY_INT_VALUE is a much better choice
int TMD::TMDgetNf()
{
    /** TMDlib helper to return Nr of flavors from info file */
    return stringToInt(getValue(TMDdict, "NumFlavors"));
    // cout << " TMDgetNF: " << TMDdict.find("NumFlavors")->second << " index = " << index <<  endl;
}
// comment: currently if NumMembers does not exist, this function is crashing. so I think returning DEFAULT_INVALID_KEY_INT_VALUE is a much better choice
int TMD::TMDgetNumMembers()
{
    /** TMDlib helper to return Nr of Member from info file */
    return stringToInt(getValue(TMDdict, "NumMembers"));
    // cout << " TMDgetNumMembers: " << TMDdict.find("NumMembers")->second << " index = " << index <<  endl;
}
// comment: currently if AlphaS_OrderQCD does not exist, this function is crashing. so I think returning DEFAULT_INVALID_KEY_INT_VALUE is a much better choice
int TMD::TMDgetOrderAlphaS()
{
    /** TMDlib helper to return order of alphas (nr of loops) from info file */
    return stringToInt(getValue(TMDdict, "AlphaS_OrderQCD"));
    // cout << " TMDgetOrderAlphas: " << TMDdict.find("AlphaS_OrderQCD")->second << " index = " << index <<  endl;
}
// comment: currently if OrderQCD does not exist, this function is crashing. so I think returning DEFAULT_INVALID_KEY_INT_VALUE is a much better choice
int TMD::TMDgetOrderPDF()
{
    /** TMDlib helper to return order of evolution (nr of loops) from info file */
    return stringToInt(getValue(TMDdict, "OrderQCD"));
...
double TMD::TMDgetXmin()
{
    ...
    std::string XMin = getValue(TMDdict, "XMin", "NOT_FOUND");
    if (XMin == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] XMin is necessary for the info file");
    }
    double XMinVal = stringToDouble(XMin, -1.);
    if (XMinVal < 0)
    {
        throw std::runtime_error("[TMDLIB2] XMin cannot be negative");
    }
    m_xMin = XMinVal;
    return XMinVal;
}
...
double TMD::TMDgetXmax()
{
    ...
    std::string XMax = getValue(TMDdict, "XMax", "NOT_FOUND");
    if (XMax == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] XMax is necessary for the info file");
    }
    double XMaxVal = stringToDouble(XMax, -1.);
    if (XMaxVal < 0)
    {
        throw std::runtime_error("[TMDLIB2] XMax cannot be negative");
    }
    m_xMax = XMaxVal;
    return XMaxVal;
}

double TMD::TMDgetKtmin()
{
    if (m_ktMin != -1)
        return m_ktMin;
    /** TMDlib helper to return XMin of TMD parametrization from info file */
    std::string ktMin = getValue(TMDdict, "KtMin", "NOT_FOUND");
    if (ktMin == "NOT_FOUND")
    {
        // SV19_nnlo uses KtoQMin
        ktMin = getValue(TMDdict, "KtoQMin", "NOT_FOUND");
        if (ktMin == "NOT_FOUND")
            throw std::runtime_error("[TMDLIB2] ktMin is necessary for the info file");
    }
    double ktMinVal = stringToDouble(ktMin, -1.);
    if (ktMinVal < 0)
    {
        throw std::runtime_error("[TMDLIB2] ktMin cannot be negative");
    }
    m_ktMin = ktMinVal;
    return ktMinVal;
}

double TMD::TMDgetKtmax()
{
    if (m_ktMax != -1)
        return m_ktMax;
    /** TMDlib helper to return XMax of TMD parametrization from info file */
    std::string ktMax = getValue(TMDdict, "KtMax", "NOT_FOUND");
    if (ktMax == "NOT_FOUND")
    {
        {
            ktMax = getValue(TMDdict, "KtoQMax", "NOT_FOUND");
            if (ktMax == "NOT_FOUND")
                throw std::runtime_error("[TMDLIB2] ktMax is necessary for the info file");
        }
    }
    double ktMaxVal = stringToDouble(ktMax, -1.);
    if (ktMaxVal < 0)
    {
        throw std::runtime_error("[TMDLIB2] ktMax cannot be negative");
    }
    m_ktMax = ktMaxVal;
    return ktMaxVal;
}
// comment: currently if QMin does not exist, this function is crashing. so I think returning DEFAULT_INVALID_KEY_DOUBLE_VALUE is a much better choice
double TMD::TMDgetQmin()
{
    /** TMDlib helper to return QMin of TMD parametrization from info file */
    if (m_QMin != -1)
        return m_QMin;
    std::string QMin = getValue(TMDdict, "QMin", "NOT_FOUND");
    if (QMin == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] QMin is necessary for the info file");
    }
    double QMinVal = stringToDouble(QMin, -1.);
    if (QMinVal < 0)
    {
        throw std::runtime_error("[TMDLIB2] QMin cannot be negative");
    }
    m_QMin = QMinVal;
    return QMinVal;
}
...
double TMD::TMDgetQ2min()
{
    ...
    m_QMin = QMinVal;
    return QMinVal * QMinVal;
}
double TMD::TMDgetQ2max()
{
    /** TMDlib helper to return Q2Max of TMD parametrization from info file */
    if (m_QMax != -1)
        return m_QMax * m_QMax;
    std::string QMax = getValue(TMDdict, "QMax", "NOT_FOUND");
    if (QMax == "NOT_FOUND")
    {
        throw std::runtime_error("[TMDLIB2] QMax is necessary for the info file");
    }
    // cout << " TMDgetQ2min as double : " << QMin << endl;
    double QMaxVal = stringToDouble(QMax, -1.);
    if (QMaxVal < 0)
    {
        throw std::runtime_error("[TMDLIB2] QMin cannot be negative");
    }
    m_QMax = QMaxVal;
    return QMaxVal * QMaxVal;
}
...
std::string TMD::TMDgetExtrapolation_Q2()
{
    ...
    return getValue(TMDdict, "Extrapolation_Q2");
}
std::string TMD::TMDgetExtrapolation_kt()
{
    /** TMDlib helper to return Extrapolation of TMD parametrization from info file */
    return getValue(TMDdict, "Extrapolation_kt");
}
std::string TMD::TMDgetExtrapolation_x()
{
    /** TMDlib helper to return Extrapolation of TMD parametrization from info file */
    return getValue(TMDdict, "Extrapolation_x");
}

int TMD::TMDnumberPDF(const std::string &name)
{
    ...
    if (name == "")
    {
        std::cout << " TMDnumberPDF: name is not defined " << name << std::endl;
        return DEFAULT_INVALID_KEY_INT_VALUE;
    }

    else
    {
        // if SetIndex does not exist, instead of throwing exception now return DEFAULT_INVALID_KEY_INT_VALUE
        int index = stringToInt(getValue(TMDdict, "SetIndex"));
...
std::string TMD::TMDstringPDF(int index)
{
    ...
    if (index == 0)
    {
        std::cout << " TMDstringPDF: iset is not defined " << index << std::endl;
        return error;
    }
    ...
    else
    {
        ...
        if (TMDindex.empty())
        { // The map needs to be populated first
            char *pathsvar = std::getenv("TMDLIBPATH");
            std::string filepath;
            // But fall back to looking in LHAPATH if the preferred var is not defined
            if (pathsvar == nullptr)
                pathsvar = getenv("TMDLIBOATH");
            const std::string spathsvar = (pathsvar != 0) ? pathsvar : "";
            // Split the paths variable as usual
            std::vector<std::string> rtn = split(spathsvar, ":");
            // Look in the install prefix after other paths are exhausted, if not blocked by a trailing ::
            ...
            if (indexpath.empty())
            {
                std::cout << " TMDstringPDF: Could not find a tmdpdfsets.index file" << std::endl;
            };
            try
            {
...
std::string TMD::getValue(const std::map<std::string, std::string> &map, const std::string &key, const std::string &defaultValue)
{
    auto foundItr = map.find(key);
    return (foundItr != map.end()) ? foundItr->second : defaultValue;
}
