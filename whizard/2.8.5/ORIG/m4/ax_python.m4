...
if test x$ax_python_bin = x; then
   ...
   $PYTHON_BIN --version >>conftest.log 2>&1

   version_string=`$GREP -i 'Python' conftest.log | head -1` 
   ax_python_major_version=[`echo $version_string | $SED -n -e 's|Python .*\([0-9][0-9]*\.[0-9][0-9]*\)\.[0-9][0-9]*|\1|p'`]
   rm -f conftest.log
   PYTHON_MAJOR_VERSION=$ax_python_major_version
...
AC_MSG_RESULT([    Major version: $ax_python_major_version])
